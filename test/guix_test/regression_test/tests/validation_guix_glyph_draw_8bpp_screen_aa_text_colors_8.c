...
TEST_PARAM test_parameter = {
    ...
    55, 107, 560, 300  /* Define the coordinates of the capture area.
...
#include "demo_glyph_draw_8bpp.c"

int clip_width[] = {5, 10, 15, 20, 25, 30};

char test_comment[256];
extern GX_STUDIO_DISPLAY_INFO glyph_draw_8bpp_screen_display_table[];
extern VOID _gx_display_driver_8bpp_glyph_4bit_draw(GX_DRAW_CONTEXT *context, GX_RECTANGLE *draw_area, GX_POINT *map_offset, GX_CONST GX_GLYPH *glyph);
...
GX_CONST GX_THEME *theme_ptr;
    ...
    /* Save palette. */
    theme_ptr = glyph_draw_8bpp_screen_display_table[DISPLAY_1].theme_table[0];
    ...
    for(test_case = 0; test_case < 2; test_case++)
    {
        if(test_case == 1)
        {
            /* Change 4bit glyph draw driver. */
            root->gx_window_root_canvas->gx_canvas_display->gx_display_driver_4bit_glyph_draw = _gx_display_driver_8bpp_glyph_4bit_draw;
        }
