...
TEST_PARAM test_parameter = {
    "guix_synergy_4444argb_16bpp", /* Test name */
    0, 0, 320, 240  /* Define the coordinates of the capture area.
...
#include "demo_synergy_4444argb_16bpp.c"
...
static int format_value[] = {GX_TRUE, GX_FALSE};
...
static VOID control_thread_entry(ULONG input)
{
    ...
    int frame_id = 1;
        ...
        for(compressed_index = 0; compressed_index <(int)(sizeof(compressed_value)/sizeof(int)); compressed_index++)
        {
		...
		format = format_value[format_index];
	        ...
	        */
		    ...
		    if(format)
		    {
		        sprintf(test_comment, "compressed for 4444ARGB ");
		    }
		    else
		    {
		        sprintf(test_comment, "compressed for 4444BGRA ");
		    }
	        }
	        else
	        {
