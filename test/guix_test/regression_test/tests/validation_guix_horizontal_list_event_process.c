...
TEST_PARAM test_parameter = {
    "guix_horizontal_list_event_process", /* Test name */
    54, 352, 300, 451  /* Define the coordinates of the capture area.
...
INT                 index;
    ...
    /* Resize to make child width < widget width. */
    gx_utility_rectangle_define(&size, 54, 352, 300, 451);
    ...
    for(index = 0; index < 9; index++)
    {
        ...
        case 1:
            my_event.gx_event_type = GX_EVENT_HORIZONTAL_FLICK;
            gx_system_event_send(&my_event);
            sprintf(test_comment, "send horizontal event");
            break;
        ...
        case 3:
         ...
         case 4:
            my_event.gx_event_type = GX_EVENT_PEN_DRAG;
            my_event.gx_event_payload.gx_event_pointdata.gx_point_x = size.gx_rectangle_left + 20;
            gx_system_event_send(&my_event);
            sprintf(test_comment, "send pen drag event");
            break;
         ...
         case 7:
            gx_horizontal_list_selected_set(list, list->gx_horizontal_list_child_count - 1);
