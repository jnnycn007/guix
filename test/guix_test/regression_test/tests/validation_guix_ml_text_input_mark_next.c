...
TEST_PARAM test_parameter = {
    "guix_ml_text_input_mark_next", /* Test name */
...
GX_EVENT                  my_event;
    ...
    my_event.gx_event_payload.gx_event_shortdata[0] = GX_KEY_HOME;
    gx_system_event_send(&my_event);

    my_event.gx_event_type = GX_EVENT_KEY_DOWN;
    my_event.gx_event_payload.gx_event_shortdata[0] = GX_KEY_UP_ARROW;
    gx_system_event_send(&my_event);

    gx_validation_set_frame_id(frame_id++);
    sprintf(test_comment, "move cursor to the start of the text");
    gx_validation_set_frame_comment(test_comment);
    gx_system_event_send(&my_event);
    ...
    for(index = 0; index <= (int)(strlen(test_string)); index++)
    {
        ...
        gx_validation_set_frame_id(frame_id++);
        sprintf(test_comment, "mark next");
        gx_validation_set_frame_comment(test_comment);
        gx_validation_screen_refresh();
    }
