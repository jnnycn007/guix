...
TEST_PARAM test_parameter = {
    "guix_animation_complete", /* Test name */
...
GX_EXIT_CRITICAL
    ...
    gx_widget_status_add((GX_WIDGET *)&button, GX_STATUS_STUDIO_CREATED);
    sprintf(test_comment[0], "Add GX_STATUS_STUDIO_CREATED status, start canvas animation");
    tx_thread_sleep(1);
    gx_animation_start(&animation, &info);
    ...
    gx_validation_set_frame_comment("Start animation and detach animation target from its parent");
GX_ENTER_CRITICAL
...
GX_EXIT_CRITICAL
    ...
    gx_validation_set_frame_comment("Sleep to wait animation complete");
