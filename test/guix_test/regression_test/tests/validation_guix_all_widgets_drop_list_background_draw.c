...
TEST_PARAM test_parameter = {
    "guix_all_widgets_drop_list_background_draw", /* Test name */
...
int                        frame_id = 1;
    ...
    gx_widget_style_remove(drop_list, GX_STYLE_TILE_BACKGROUND);
    sprintf(test_comment, "Set pixelmap fish to drop list.");
    gx_system_dirty_mark(drop_list);
    gx_validation_set_frame_id(frame_id++);
    gx_validation_set_frame_comment(test_comment);
    ...
    gx_widget_status_add(drop_list, GX_STATUS_HAS_FOCUS);
    sprintf(test_comment, "Set pixelmap fish to drop list.");
    gx_system_dirty_mark(drop_list);
    gx_validation_set_frame_id(frame_id++);
    gx_validation_set_frame_comment(test_comment);
