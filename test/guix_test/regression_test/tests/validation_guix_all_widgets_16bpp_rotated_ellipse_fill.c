...
TEST_PARAM test_parameter = {
    "guix_all_widgets_16bpp_rotated_ellipse_fill", /* Test name */
...
ELLIPSE_INFO ellipse_info_list[]={
{213, 230, 300, 100},
{213, 230, 100, 300},
{323, 230, 120, 120},
{213, 230, 35, 5},
...
static int test_radius_a;
static int test_radius_b;
...
UINT style = GX_BRUSH_SOLID_FILL | GX_BRUSH_ALIAS;
    ...
    gx_canvas_ellipse_draw(test_xcenter, test_ycenter, test_radius_a, test_radius_b);
}
...
GX_BRUSH *brush;
    ...
    gx_canvas_ellipse_draw(213, 300, 160, 100);
...
ELLIPSE_INFO *info; 
    ...
    {
        info = ellipse_info_list; 

        while(info->radius_a)
        {
            test_xcenter = info->xcenter;
            test_ycenter = info->ycenter;
            test_radius_a = info->radius_a;
            test_radius_b = info->radius_b;

            sprintf(test_comment, "fill map = %d, xcenter = %d, ycenter = %d, a = %d, b = %d",
                    test_fill_map, info->xcenter, info->ycenter, info->radius_a, info->radius_b);
