...
TEST_PARAM test_parameter = {
    "guix_ml_text_input_api_select_invisible", /* Test name */
...
GX_EVENT                  my_event;
    ...
    for(index = 0; index < 3; index++)
    {
        ...
        {
        case 0:
            sprintf(test_comment, "hide input widget");
            gx_widget_hide((GX_WIDGET *)text_input);
            break;
        case 1:
            sprintf(test_comment, "select text[0, 5]");
            gx_multi_line_text_input_text_select(text_input, 0, 5);
            break;

        case 2:
            sprintf(test_comment, "show input widget");
