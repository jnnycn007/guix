...
TEST_PARAM test_parameter = {
    "guix_sl_input_backspace", /* Test name */
...
static GX_CONST GX_UBYTE SINGLE_LINE_TEXT[] = "do you remember the day we used to be, why do the";
static INT key_list[]={GX_KEY_HOME, GX_KEY_RIGHT_ARROW, GX_KEY_BACKSPACE, GX_KEY_RIGHT_ARROW, GX_KEY_BACKSPACE};
static GX_CHAR *test_comments[5] = {"home", "right arrow", "backspace", "right arrow", "backspace"};
...
GX_EVENT my_event;
    ...
    for(index = 0; index < (int)(sizeof(key_list) / sizeof(INT)); index++)
    {
        my_event.gx_event_type = GX_EVENT_KEY_DOWN;
        my_event.gx_event_payload.gx_event_ushortdata[0] = key_list[index];
        gx_system_event_send(&my_event);

        gx_validation_set_frame_id(frame_id++);
        gx_validation_set_frame_comment(test_comments[index]);
