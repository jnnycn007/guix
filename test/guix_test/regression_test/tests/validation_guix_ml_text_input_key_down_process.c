...
TEST_PARAM test_parameter = {
    "guix_ml_text_input_key_down_process", /* Test name */
...
GX_EVENT                  my_event;
    ...
    sprintf(test_comment, "insert a Chinese characer");
    gx_validation_set_frame_comment(test_comment);

    my_event.gx_event_display_handle = 1;
    my_event.gx_event_type = GX_EVENT_KEY_DOWN;
    my_event.gx_event_payload.gx_event_ushortdata[0] = 0x4e2d;
    gx_system_event_send(&my_event);

    /* Insert a control character. */
    my_event.gx_event_type = GX_EVENT_KEY_DOWN;
    my_event.gx_event_payload.gx_event_ushortdata[0] = 0x1e;
    gx_system_event_send(&my_event);

    gx_validation_set_frame_id(frame_id++);
    sprintf(test_comment, "insert a control character");
    gx_validation_set_frame_comment(test_comment);
