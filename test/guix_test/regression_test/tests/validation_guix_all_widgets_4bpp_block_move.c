...
TEST_PARAM test_parameter = {
    "guix_all_widgets_4bpp_block_move", /* Test name */
...
GX_RECTANGLE block;
    ...
    gx_utility_rectangle_define(&block, 57, 217, 100, 301);
    gx_validation_set_frame_id(frame_id++);
    gx_validation_set_frame_comment("block right");
    gx_widget_block_move((GX_WIDGET *)&window_screen.window_screen_vertical_list, &block, 10, 0);

    gx_validation_set_frame_id(frame_id++);
    gx_validation_set_frame_comment("block left");
    gx_widget_block_move((GX_WIDGET *)&window_screen.window_screen_vertical_list, &block, -10, 0);
