...
TEST_PARAM test_parameter = {
    "guix_ml_text_input_backspace_16bpp", /* Test name */
...
char test_string[]="why do the birds go on singing, why do the seas rush the shore.";
...
GX_EVENT                  my_event;
    ...
    for(index = 0; index < (int)strlen(test_string); index++)
    {
        gx_validation_set_frame_id(frame_id++);
        sprintf(test_comment, "Input character:%c", test_string[index]);
        gx_validation_set_frame_comment(test_comment);
    ...
    {
        gx_validation_set_frame_id(frame_id++);
        sprintf(test_comment, "Move cursor up by lines.");
        gx_validation_set_frame_comment(test_comment);

        /* Move cursor up. */
        my_event.gx_event_type = GX_EVENT_KEY_DOWN;
        my_event.gx_event_payload.gx_event_ushortdata[0] = GX_KEY_UP_ARROW;
        gx_system_event_send(&my_event);
    }

    gx_validation_set_frame_id(frame_id++);
    sprintf(test_comment, "\"Home\" key down.");
    gx_validation_set_frame_comment(test_comment);
    ...
    {
        gx_validation_set_frame_id(frame_id++);
        sprintf(test_comment, "\"Backspace\" key down.");
        gx_validation_set_frame_comment(test_comment);
    ...
    gx_multi_line_text_input_buffer_get(text_input, GX_NULL, GX_NULL, GX_NULL);
    gx_multi_line_text_input_buffer_clear(text_input);
    gx_multi_line_text_input_buffer_clear(text_input);

    /* Insert string with 35 lines. */
    for(index = 0; index < 36; index++)
    {
        gx_validation_set_frame_id(frame_id++);
        sprintf(test_comment, "insert %d", index);
        gx_validation_set_frame_comment(test_comment);

        my_event.gx_event_type = GX_EVENT_KEY_DOWN;
        my_event.gx_event_payload.gx_event_shortdata[0] = '0' + index;
        gx_system_event_send(&my_event);

        gx_validation_set_frame_id(frame_id++);
        sprintf(test_comment, "insert return character");
        gx_validation_set_frame_comment(test_comment);

        my_event.gx_event_payload.gx_event_shortdata[0] = '\r';
        gx_system_event_send(&my_event);
    }
    ...
    {
        gx_validation_set_frame_id(frame_id++);
        sprintf(test_comment, "left arrow key dowm");
        gx_validation_set_frame_comment(test_comment);

        my_event.gx_event_type = GX_EVENT_KEY_DOWN;
        my_event.gx_event_payload.gx_event_shortdata[0] = GX_KEY_LEFT_ARROW;
        gx_system_event_send(&my_event);
    }
    ...
    for(index = 1; index <= 5; index++)
    {
        ...
        case 4:
            ...
            while(count)
            {
                gx_validation_set_frame_comment(test_comment);
                my_event.gx_event_type = GX_EVENT_KEY_DOWN;
                my_event.gx_event_payload.gx_event_shortdata[0] = GX_KEY_BACKSPACE;
                gx_system_event_send(&my_event);
                count--;
            }
            break;
