...
TEST_PARAM test_parameter = {
    "guix_ml_text_view_display_info_get", /* Test name */
...
GX_MULTI_LINE_TEXT_VIEW *view;
    ...
    gx_utility_rectangle_define(&size, 50, 119, 77, 316);
    gx_widget_resize((GX_WIDGET *)view, &size);

    gx_validation_set_frame_id(frame_id++);
    sprintf(test_comment, "Resize ml_text_view to make the widget client widget smaller than one character width");
