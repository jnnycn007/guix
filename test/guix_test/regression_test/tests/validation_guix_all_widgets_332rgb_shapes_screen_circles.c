...
TEST_PARAM test_parameter = {
    "guix_all_widgets_332rgb_shapes_screen_circles", /* Test name */
...
{
int a,b,c,i,j,pixelmap_id_index, brush_alpha_index;
int frame_id = 1;

    /*draw shape is circle.*/
    draw_shape = 0;
    /*radius width*/
                        ...
                        for(brush_alpha_index = 0; brush_alpha_index < (INT)(sizeof(brush_alpha_value)/sizeof(INT)); brush_alpha_index++)
                        {
                            ...
                            if(pixelmap_fill)
                            {
                                for(pixelmap_id_index = 0; pixelmap_id_index < (INT)(sizeof(fill_pixelmap_id)/sizeof(GX_RESOURCE_ID)); pixelmap_id_index++)
                                {
                                    ...
                                    sprintf(test_comment, "%d, brush_width %d, radius %d, fill_pixelmap_id: %d, brush_alpha: %d, ", frame_id, brush_width, radius, pixelmap_id_index, brush_alpha);
                            ...
                            else
                            {
                                ...
                                */
                                gx_validation_set_frame_id(frame_id);
                                sprintf(test_comment, "%d, brush_width %d, radius %d, brush_alpha: %d,", frame_id, brush_width, radius, brush_alpha);
                                if(pixelmap_fill){strcat(test_comment, "pixelmap_fill, ");}
                                else{strcat(test_comment, " without pixelmap_fill, ");}
                                if(solid_fill){strcat(test_comment, "solid_fill, ");}
                                else{strcat(test_comment, " without solid_fill, ");}
