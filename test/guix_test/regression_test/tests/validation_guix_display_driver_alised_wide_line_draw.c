...
TEST_PARAM test_parameter = {
    "guix_display_driver_alised_wide_line_draw", /* Test name */
...
GX_RECTANGLE *size;
    ...
    line_x1 = window->gx_widget_size.gx_rectangle_left;
    line_x2 = line_x1;
    line_y1 = window->gx_widget_size.gx_rectangle_top - 5;
    line_y2 = line_y1;
    gx_canvas_line_draw(line_x1, line_y1, line_x2, line_y2);

    line_x1 = window->gx_widget_size.gx_rectangle_left - 5;
    line_x2 = line_x1;
    line_y1 = window->gx_widget_size.gx_rectangle_top;
    line_y2 = window->gx_widget_size.gx_rectangle_bottom;
    gx_canvas_line_draw(line_x1, line_y1, line_x2, line_y2);
}
...
GX_DISPLAY *display;
    ...
    lines_screen_line_width = 10;
    lines_screen_line_length = 300;
    lines_screen_anti_aliased = GX_TRUE;
    lines_screen_rounded = GX_TRUE;
    for(index = 0; index <= 1; index++)
    {
        if(index) display->gx_display_driver_pixel_blend = 0;
        for(lines_screen_brush_alpha = 255; lines_screen_brush_alpha > 100; lines_screen_brush_alpha -= 100)
        {
             for(lines_screen_line_angle = 0; lines_screen_line_angle < 360; lines_screen_line_angle+=30)
             {
                 ...
                 */
                 ...
                 sprintf(test_comment, "brush alpha = %d, line length = %d, angle = %d, line width: %d, ", 
                 lines_screen_brush_alpha, lines_screen_line_length, lines_screen_line_angle, lines_screen_line_width);
                 if(index) strcat(test_comment, "display_driver_pixel_blend is set to NULL.");
