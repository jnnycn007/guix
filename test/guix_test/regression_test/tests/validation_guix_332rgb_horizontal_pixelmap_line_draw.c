...
TEST_PARAM test_parameter = {
    "guix_332rgb_horizontal_pixelmap_line_draw", /* Test name */
...
GX_PIXELMAP_ID_SAVE_ICON,/*compressed*/
GX_PIXELMAP_ID_I_ORANGEFILL_BOTTOM,/*compressed, alpha*/
};
        ...
        for(test_shape = 0; test_shape < 2; test_shape++)
        {
            for(blend_alpha = 128; blend_alpha <= 255; blend_alpha += 127)
            {
                for(index = 0; index < (int)(sizeof(map_id_list)/sizeof(int)); index++)
                {
                    ...
                    sprintf(test_comment, "set_pixel_blend_null = %d, blend_alpha = %d, map_id = %d, shape id = %d",
                            set_blend_null, blend_alpha, map_id, test_shape);
