...
TEST_PARAM test_parameter = {
    "guix_all_widgets_32argb_rotated_cw_pixelmap_draw", /* Test name */
...
INT display_color_format;
    ...
    for(index = 0; index < 2; index++)
    {
        ...
        {
            display_color_format = GX_COLOR_FORMAT_32ARGB;
        }
        ...
        for(test_brush_alpha = 255; test_brush_alpha > 0; test_brush_alpha -= 128)
        {
            ...
            while(map_info->map_id)
            {
                ...
                sprintf(test_comment, "brush alpha = %d, map id = %s", test_brush_alpha, map_info->map_name);
