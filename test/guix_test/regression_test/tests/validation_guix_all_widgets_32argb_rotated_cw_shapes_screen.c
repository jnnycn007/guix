...
TEST_PARAM test_parameter = {
    "guix_all_widgets_32argb_rotated_cw_shapes_screen", /* Test name */
...
INT alpha_index;
    ...
    for(alpha_index = 0; alpha_index < TEST_ALPHA_CASE; alpha_index++ )
    {
        ...
        {
            for(fill_pixelmap_index = 0; fill_pixelmap_index < 3; fill_pixelmap_index++)
            {
                sprintf(test_comment, "brush alpha = %d, shape id = %d. ", brush_alpha, draw_shape);

                if(fill_pixelmap_index < 2)
                {
