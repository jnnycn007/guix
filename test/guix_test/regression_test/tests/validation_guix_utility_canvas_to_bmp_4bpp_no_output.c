...
TEST_PARAM test_parameter = {
    "guix_utility_canvas_to_bmp_4bpp_no_output", /* Test name */
...
#include "demo_guix_screenshot_to_bitmap_4bpp.c"
...
int  pathlen;
char relative_path[]="../../../example_internal/screenshot_to_bitmap/screenshot_to_bitmap_4bpp/";
    ...
    sprintf(golden_file + pathlen, "%s%s", relative_path, golden_bmp_file_aligned);
    sprintf(output_file + pathlen, "%s%s", relative_path, bmp_file_aligned);
    status = gx_validation_compare_file_data(output_file, golden_file);
    remove(output_file);

    remove(bmp_file);
    remove(scroll_wheel_bmp_file);
    remove(bmp_file_aligned);
