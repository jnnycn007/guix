...
TEST_PARAM test_parameter = {
    "guix_pixelmap_slider_24xrgb", /* Test name */
...
GX_RECTANGLE new_size;
    ...
    gx_widget_find((GX_WIDGET *) pIndicatorScreen, ID_PIXELMAP_SLIDER1, GX_SEARCH_DEPTH_INFINITE, (GX_WIDGET **) &slider);
    ...
    for (increment_index = 1; increment_index >= 0; increment_index --)
    {
        ...
        for (min_travel = 10; min_travel < slider_height; min_travel += 10)
        {
            for (max_travel = 10; max_travel < (slider_height - min_travel); max_travel +=20)
            {
                for(pos = slider_top; pos <= slider_bottom; pos += 20)
                {
                    ...
                    */
                    ...
                    if (increment_index)
                    {
                        strcat(test_comment, ", shift to right, reset value");
                    }
                    else
                    {
                        strcat(test_comment, ", shift to left, resize, reset value");
                    }
                    ...
                    case 1:
                        ...
                        strcat(test_comment, ", tiled, with lowermap and uppermap");
                        backgd_index ++;
                        break;
                    case 2:
                        pixinfo -> gx_pixelmap_slider_info_needle_pixelmap = GX_PIXELMAP_ID_I_INDICATOR;
                        pixinfo -> gx_pixelmap_slider_info_upper_background_pixelmap = GX_NULL;
                        gx_pixelmap_slider_pixelmap_set(slider, pixinfo);
                        strcat(test_comment, ", tiled, with lowermap");
                        backgd_index ++;
                        break;
                    case 3:
                    ...
                    event_ptr->gx_event_payload.gx_event_pointdata.gx_point_x = 170;
