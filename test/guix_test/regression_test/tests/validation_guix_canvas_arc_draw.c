...
TEST_PARAM test_parameter = {
    "guix_canvas_arc_draw", /* Test name */
...
GX_RECTANGLE size;
    ...
    sprintf(test_comment, "Set brush properties and draw original arc. start_angle: %d, end_angle: %d.", start_angle, end_angle);
    ...
    /* Set frame id. */
    gx_validation_set_frame_id(frame_id++);
    sprintf(test_comment, "Add a window to cover view where the arc should be draw.");
    gx_validation_set_frame_comment(test_comment);
    gx_system_dirty_mark(pShapesScreen);
    gx_validation_screen_refresh();
    ...
    /* Set frame id. */
    gx_validation_set_frame_id(frame_id++);
    sprintf(test_comment, "Draw arc outside the window client.");
    gx_validation_set_frame_comment(test_comment);
    gx_system_dirty_mark(pShapesScreen);
    gx_validation_screen_refresh();
