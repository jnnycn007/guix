...
TEST_PARAM test_parameter = {
    "guix_multi_line_text_input_left_arrow", /* Test name */
...
GX_CHAR test_string[]=
"0 do you remember the day\r\n we used to\r\n be\r\r1\r2\r3\r4ABCDEFGH\r\nIJKLMN\r5\r6\r7\r8\r9\r\r\
0\r1\r2\r3\r4\r5OPQRSTU\r\nVWXYZ\r6\r7\r8\r9\r\
0\r1\r2\r3\r4\r5abcdefghijk\r\nlmn\r6\r7\r8\r9\r0\r\
31\r32\r33\r34\r35\r36\r37ABCDE\r\nFG\r38abcdefghijk\r39ABCDEF\r40";
...
int index = 0;
    ...
    sprintf(test_comment, "Set text to multi line text input and send pen down event.");
    ...
    while(index++ < 20)
    {
        ...
        my_event.gx_event_payload.gx_event_ushortdata[0] = GX_KEY_DOWN_ARROW;
        my_event.gx_event_target = (GX_WIDGET *)input;
        gx_system_event_send(&my_event);
    }
    gx_validation_set_frame_id(frame_id++);
    sprintf(test_comment, "Send key end event to widget 20 times.");
    gx_validation_set_frame_comment(test_comment);
    /* Force a screen refresh. */
    gx_validation_screen_refresh();
    ...
    gx_validation_set_frame_id(frame_id++);
    sprintf(test_comment, "Send key left arrow event to widget 100 times.");
    gx_validation_set_frame_comment(test_comment);
    /* Force a screen refresh. */
    gx_validation_screen_refresh();
    
    display = root->gx_window_root_canvas->gx_canvas_display;
    gx_display_font_table_set(display, GX_NULL, 0);
    gx_validation_set_frame_id(frame_id++);
    sprintf(test_comment, "Set font table of current display to NULL.");
    gx_validation_set_frame_comment(test_comment);
    /* Force a screen refresh. */
    gx_validation_screen_refresh();
    ...
    gx_system_event_send(&my_event);
    gx_validation_set_frame_id(frame_id++);
    sprintf(test_comment, "Send key left arrow event to widget.");
    gx_validation_set_frame_comment(test_comment);
    /* Force a screen refresh. */
    gx_validation_screen_refresh();
