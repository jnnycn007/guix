...
TEST_PARAM test_parameter = {
    "guix_utility_4bpp_pixelmap_resize", /* Test name */
...
#define TEST_MEMORY_SIZE_8000 8000
#define TEST_MEMORY_SIZE_500  500

GX_COLOR test_memory_8000[TEST_MEMORY_SIZE_8000];
...
int      memory_index;
    ...
    for(memory_index = 0; memory_index < 2; memory_index++)
    {
        ...
        {
            tx_byte_pool_delete(&rotate_pool);
            tx_byte_pool_create(&rotate_pool, "test_memory_pool_8000", test_memory_8000,
                        TEST_MEMORY_SIZE_8000 * sizeof(GX_COLOR));
        ...
        for(pixelmap_index = 0; pixelmap_index < 2; pixelmap_index++)
        {
            ...
            {
                strcat(test_comment, ", memory pool size = 8000 * 4(bytes)");
            }
