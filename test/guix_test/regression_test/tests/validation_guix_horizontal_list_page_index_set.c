...
TEST_PARAM test_parameter = {
    "guix_horizontal_list_page_index_set", /* Test name */
...
GX_WIDGET *selected;
    ...
    for(index = 0; index < (int)(sizeof(index_list)/sizeof(int)); index++)
    {
        ...
        if((page_index >= 0) && (page_index < total_list_cols))
        {
#endif
            gx_horizontal_list_page_index_set(&window.window_horizontal_list_scroll, page_index);
            gx_horizontal_list_page_index_set(&window.window_horizontal_list_wrap, page_index);
#if defined(GX_DISABLE_ERROR_CHECKING)
        }
#endif
        ...
        if(page_index == 19)
        {
            ...
            gx_horizontal_list_selected_widget_get(wrap_list, &selected);
            gx_horizontal_list_selected_set(wrap_list, 0);
            ...
            sprintf(test_comment, "select wrap list index 1");
            gx_validation_set_frame_id(frame_id++);
            gx_validation_set_frame_comment(test_comment);
            gx_validation_screen_refresh();

            gx_horizontal_list_selected_set(scroll_list, 1);
            gx_horizontal_list_selected_widget_get(scroll_list, &selected);
            gx_horizontal_list_selected_set(scroll_list, 0);
            ...
            sprintf(test_comment, "select scroll list index 1");
            gx_validation_set_frame_id(frame_id++);
            gx_validation_set_frame_comment(test_comment);
            gx_validation_screen_refresh();
        }
