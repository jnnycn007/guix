...
TEST_PARAM test_parameter = {
    "guix_canvas_pie_draw", /* Test name */
...
GX_RECTANGLE size;
    ...
    draw_shape = 2; //2 is pie

    /* Set frame id. */
    gx_validation_set_frame_id(frame_id++);
    sprintf(test_comment, "Set brush properties and draw original pie.");
    ...
    /* Set frame id. */
    gx_validation_set_frame_id(frame_id++);
    sprintf(test_comment, "Add a window to cover view where the pie should be draw.");
    gx_validation_set_frame_comment(test_comment);
    gx_system_dirty_mark(pShapesScreen);
    gx_validation_screen_refresh();
    ...
    /* Set frame id. */
    gx_validation_set_frame_id(frame_id++);
    sprintf(test_comment, "Draw pie outside the window client.");
    gx_validation_set_frame_comment(test_comment);
    gx_system_dirty_mark(pShapesScreen);
    gx_validation_screen_refresh();
