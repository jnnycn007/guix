...
TEST_PARAM test_parameter = {
    "guix_utility_4bpp_pixelmap_rotate", /* Test name */
    5, 68, 635, 404  /* Define the coordinates of the capture area.
...
#define TEST_MEMORY_SIZE_7000 7000
#define TEST_MEMORY_SIZE_500  500

GX_COLOR test_memory_7000[TEST_MEMORY_SIZE_7000];
...
int          memory_index;
    ...
    ToggleScreen((GX_WINDOW *)&rotate_screen, (GX_WINDOW *)&sprite_screen);
    ...
    for(memory_index = 0; memory_index < 3; memory_index++)
    {
        ...
        case 1:
            ...
            tx_byte_pool_create(&rotate_pool, "test_memory_pool_7000", test_memory_7000,
                        TEST_MEMORY_SIZE_7000 * sizeof(GX_COLOR));
            break;
        ...
        for(pixelmap_index = 0; pixelmap_index < (int)(sizeof(rotate_pixelmap_id_list) / sizeof(int)); pixelmap_index++)
        {
            ...
            for(angle_index = 0; angle_index < (int)(sizeof(angle_list)/sizeof(int)); angle_index++)
            {
                ...
                for(x_center = 0; x_center < 2; x_center++)
                {
                    ...
                    {
                    case 1:
                        strcat(test_comment, ", memory pool size = 7000 * 4(bytes)");
                        break;
                    case 2:
