...
TEST_PARAM test_parameter = {
    "guix_single_line_text_input_text_select", /* Test name */
...
GX_CHAR test_string[]=
"0 do you remember the day we used to be ABCDEFGHIJKLMN!@$@*^*%(*&($#1275384635098765";
...
GX_SINGLE_LINE_TEXT_INPUT *input;
    ...
    input = &text_screen.text_screen_text_input_1;
    gx_single_line_text_input_text_set(input, test_string);
    ...
    gx_system_event_send(&my_event);
    gx_validation_set_frame_id(frame_id++);
    sprintf(test_comment, "Send mark right event to widget.");
    gx_validation_set_frame_comment(test_comment);
    /* Force a screen refresh. */
    gx_validation_screen_refresh();
    
    gx_single_line_text_input_text_select(input, 2, 5);
    gx_validation_set_frame_id(frame_id++);
