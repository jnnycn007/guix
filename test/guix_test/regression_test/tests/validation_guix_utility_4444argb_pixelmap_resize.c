...
TEST_PARAM test_parameter = {
    "guix_utility_4444argb_pixelmap_resize", /* Test name */
...
int      pixelmap_index;
    ...
    for(pixelmap_index = 0; pixelmap_index < 2; pixelmap_index++)
    {
        ...
        /* Capture a frame. */
        gx_validation_set_frame_id(frame_id++);
        sprintf(test_comment, "Set system allocate function to return null then call resize. pixelmap_id = %d", (int)resize_pixelmap_id);
        gx_validation_set_frame_comment(test_comment);
        gx_validation_screen_refresh();
