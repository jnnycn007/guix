...
#include "keyboard_8bpp_specifications.h"
...
KEY_LAYOUT_ENTRY key_layout_1[NUM_DEFAULT_KEYS + 1] = {
    ...
    {402,   111,  GX_PIXELMAP_ID_B_KEYBOARD,  GX_PIXELMAP_ID_B_KEYBOARD_H,            0,   "Enter",  GX_KEY_CARRIAGE_RETURN}, 
...
VOID key_widget_draw(KEY_WIDGET *key)
{
    ...
    if (key->text)
    {
        if (string_length_get(key->text, MAX_KEY_TEXT_LENGTH) > 1)
        {
...
VOID key_widget_deselect(GX_WIDGET *key)
{
    /* Set style for pen down.  */
...
GX_WIDGET *target;
    ...
    case KEY_PRESS_EVENT:
        ...
        */
        gx_widget_find(frame, ID_INPUT_FIELD, 2, &target);
        key_event.gx_event_target = 
